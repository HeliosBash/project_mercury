...
def solar_query_batch(client, node, sourceids, startdate, enddate, aggregate, maxoutput, token, secret, action='local', header_mode='auto', verbose=False):
    ...
    # Build parameter string based on action (UTC vs Local)
    if action == 'utc':
        if aggregate == "None":
...
def solar_query_utc(node, sourceids, startdate, enddate, aggregate, maxoutput, token, secret, header_mode='auto', output_file=None, verbose=False):
    """UTC query function (single batch, no pagination)"""
    client = Client(token, secret)

    results, response = solar_query_batch(client, node, sourceids, startdate, enddate,
                                          aggregate, maxoutput, token, secret, 'utc', header_mode, verbose)
    ...
    if not results:
        ...
        if header_mode in ['always', 'auto']:
            output_line = "created,nodeId,sourceId"
            if output_file:
    ...
    # Discover columns with custom sorting
    columns = discover_columns_sorted([results])

    # Prepare output
    output_lines = []

    # Print header based on mode
    if header_mode == 'always' or (header_mode == 'auto' and results):
...
def solar_query_local(node, sourceids, startdate, enddate, aggregate, maxoutput, token, secret, header_mode='auto', output_file=None, verbose=False):
    """Local query function with automatic pagination"""
    client = Client(token, secret)
    ...
    while True:
        ...
        results, response = solar_query_batch(client, node, sourceids, current_start, final_end,
                                               aggregate, maxoutput, token, secret, 'local', header_mode, verbose)
...
def main():
    parser = argparse.ArgumentParser(description="Solar query with UTC or Local time support!")

    parser.add_argument("--action", required=True, choices=['utc', 'local'], 
                       help="Query action: 'utc' for UTC timestamps (single batch), 'local' for local timestamps (with pagination)")
    ...
    parser.add_argument("--startdate", required=True, type=str, 
                       help="Start date in format 'YYYY-MM-DD HH:MM:SS' (use --localstartdate for local action)")
    parser.add_argument("--enddate", required=True, type=str, 
                       help="End date in format 'YYYY-MM-DD HH:MM:SS' (use --localenddate for local action)")
    parser.add_argument("--localstartdate", type=str, 
                       help="Local start date (alias for --startdate when action=local)")
    parser.add_argument("--localenddate", type=str, 
                       help="Local end date (alias for --enddate when action=local)")
    ...
    # Handle local date aliases
    startdate = args.localstartdate if args.localstartdate else args.startdate
    enddate = args.localenddate if args.localenddate else args.enddate

    try:
        if args.action == 'utc':
            solar_query_utc(args.node, args.sourceids, startdate, enddate,
                          args.aggregate, args.maxoutput, args.token, args.secret,
                          args.header, args.output, args.verbose)
        else:  # local
            solar_query_local(args.node, args.sourceids, startdate, enddate,
                            args.aggregate, args.maxoutput, args.token, args.secret,
                            args.header, args.output, args.verbose)
    except Exception as e:
